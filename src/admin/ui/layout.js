...
import { SKINS, skinStyles } from './themes/index.js';
...
${fontLinks()}${themeInitScript(cspNonce)}<style>${baseStyles()}${pageStyles}${skinStyles()}</style>
</head>
<body>
...
export function themeInitScript(nonce) {
  const themeKey = `${PRODUCT.shortName}-theme`;
  const skinKey = `${PRODUCT.shortName}-skin`;
  const defaults = Object.fromEntries(SKINS.map((skin) => [skin.id, skin.defaultScheme]));
  return scriptTag(`(function(){try{var D=${safeScriptJson(defaults)};var s=localStorage.getItem('${skinKey}');if(!D[s]){s='primer';}document.documentElement.dataset.skin=s;var t=localStorage.getItem('${themeKey}:'+s);if(t!=='light'&&t!=='dark'){t=s==='primer'?localStorage.getItem('${themeKey}'):null;if(t!=='light'&&t!=='dark'){t=D[s];}}document.documentElement.dataset.theme=t;var l=localStorage.getItem('${PRODUCT.shortName}-lang');if(l!=='en'&&l!=='zh'){l=((navigator.language||'en').toLowerCase().indexOf('zh')===0)?'zh':'en';}document.documentElement.lang=l;}catch(e){document.documentElement.dataset.skin='primer';document.documentElement.dataset.theme='light';document.documentElement.lang='en';}})();`, nonce);
}

export function togglesHtml() {
  const skinOptions = SKINS.map((skin) => `<option value="${skin.id}" data-i18n="${skin.labelKey}">${skin.id[0].toUpperCase() + skin.id.slice(1)}</option>`).join('');
  const skinPicker = `<select class="toggle-btn skin-picker" data-skin-picker data-act="pick-skin" aria-label="Skin" data-i18n-aria-label="skin">${skinOptions}</select>`;
  return `<div class="toggles">${skinPicker}<button type="button" class="toggle-btn" data-act="toggle-theme" data-theme-target aria-label="Toggle theme"><svg class="ic theme-moon" viewBox="0 0 16 16" width="16" height="16" fill="currentColor" aria-hidden="true"><path d="M9.598 1.591a.749.749 0 0 1 .785-.175 7.001 7.001 0 1 1-8.967 8.967.75.75 0 0 1 .961-.96 5.5 5.5 0 0 0 7.046-7.046.75.75 0 0 1 .175-.786Zm1.616 1.945a7 7 0 0 1-7.678 7.678 5.499 5.499 0 1 0 7.678-7.678Z"/></svg><svg class="ic theme-sun" viewBox="0 0 16 16" width="16" height="16" fill="currentColor" aria-hidden="true"><path d="M8 12a4 4 0 1 1 0-8 4 4 0 0 1 0 8Zm0-1.5a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5Zm5.657-8.157a.75.75 0 0 1 0 1.061l-1.061 1.06a.749.749 0 0 1-1.275-.326.749.749 0 0 1 .215-.734l1.06-1.06a.75.75 0 0 1 1.06 0Zm-9.193 9.193a.75.75 0 0 1 0 1.06l-1.06 1.061a.75.75 0 1 1-1.061-1.06l1.06-1.061a.75.75 0 0 1 1.061 0ZM8 0a.75.75 0 0 1 .75.75v1.5a.75.75 0 0 1-1.5 0V.75A.75.75 0 0 1 8 0ZM3 8a.75.75 0 0 1-.75.75H.75a.75.75 0 0 1 0-1.5h1.5A.75.75 0 0 1 3 8Zm13 0a.75.75 0 0 1-.75.75h-1.5a.75.75 0 0 1 0-1.5h1.5A.75.75 0 0 1 16 8Zm-8 5a.75.75 0 0 1 .75.75v1.5a.75.75 0 0 1-1.5 0v-1.5A.75.75 0 0 1 8 13Zm3.536-1.464a.75.75 0 0 1 1.06 0l1.061 1.06a.75.75 0 0 1-1.06 1.061l-1.061-1.06a.75.75 0 0 1 0-1.061ZM2.343 2.343a.75.75 0 0 1 1.061 0l1.06 1.061a.751.751 0 0 1-.018 1.042.751.751 0 0 1-1.042.018l-1.06-1.06a.75.75 0 0 1 0-1.06Z"/></svg></button><button type="button" class="toggle-btn" data-act="toggle-lang" data-lang-target>中文</button></div>`;
}
...
export function bodyScript(nonce) {
  const themeKey = `${PRODUCT.shortName}-theme`;
  const skinKey = `${PRODUCT.shortName}-skin`;
  const skins = SKINS.map((skin) => ({ id: skin.id, scheme: skin.defaultScheme }));
  return scriptTag(`(function(){var SKINS=${safeScriptJson(skins)};var I18N=${safeScriptJson(I18N)};function dict(){return I18N[document.documentElement.lang]||I18N.en;}function applyLang(){var d=dict();document.querySelectorAll('[data-i18n]').forEach(function(el){var k=el.getAttribute('data-i18n');if(d[k]!==undefined)el.textContent=d[k];});document.querySelectorAll('[data-i18n-aria-label]').forEach(function(el){var k=el.getAttribute('data-i18n-aria-label');if(d[k]!==undefined)el.setAttribute('aria-label',d[k]);});document.querySelectorAll('[data-i18n-placeholder]').forEach(function(el){var k=el.getAttribute('data-i18n-placeholder');if(d[k]!==undefined)el.setAttribute('placeholder',d[k]);});document.querySelectorAll('[data-i18n-template]').forEach(function(el){var t=d[el.getAttribute('data-i18n-template')];if(t!==undefined){el.textContent=t.split('{page}').join(el.getAttribute('data-page')||'').split('{total-pages}').join(el.getAttribute('data-total-pages')||'').split('{total}').join(el.getAttribute('data-total')||'').split('{visible}').join(el.getAttribute('data-visible')||'');}});document.querySelectorAll('[data-theme-target]').forEach(function(b){var light=document.documentElement.dataset.theme==='light';b.setAttribute('aria-label',light?d.toggle_theme_dark:d.toggle_theme_light);});document.querySelectorAll('[data-lang-target]').forEach(function(b){var zh=document.documentElement.lang==='zh';b.textContent=zh?'EN':'中文';b.setAttribute('aria-label',zh?d.toggle_lang_en:d.toggle_lang_zh);});document.querySelectorAll('[data-skin-picker]').forEach(function(sel){sel.value=document.documentElement.dataset.skin||'primer';});}function setLang(l){document.documentElement.lang=l;try{localStorage.setItem('${PRODUCT.shortName}-lang',l);}catch(e){}applyLang();}function setTheme(t){document.documentElement.dataset.theme=t;try{localStorage.setItem('${themeKey}:'+(document.documentElement.dataset.skin||'primer'),t);}catch(e){}applyLang();}function setSkin(id){var skin=null;SKINS.forEach(function(s){if(s.id===id)skin=s;});if(!skin)return;document.documentElement.dataset.skin=skin.id;var t=null;try{localStorage.setItem('${skinKey}',skin.id);t=localStorage.getItem('${themeKey}:'+skin.id);}catch(e){}if(t!=='light'&&t!=='dark'){t=skin.scheme;}document.documentElement.dataset.theme=t;try{localStorage.setItem('${themeKey}:'+skin.id,t);}catch(e){}applyLang();}document.addEventListener('click',function(e){var n=e.target.closest&&e.target.closest('[data-act]');if(!n)return;var a=n.getAttribute('data-act');if(a==='toggle-lang')setLang(document.documentElement.lang==='zh'?'en':'zh');else if(a==='toggle-theme')setTheme(document.documentElement.dataset.theme==='light'?'dark':'light');});document.addEventListener('change',function(e){var n=e.target.closest&&e.target.closest('[data-act="pick-skin"]');if(!n)return;setSkin(n.value);});applyLang();})();`, nonce);
}
...
export function baseStyles() {
  return `:root {
  color-scheme: light;
  /* ── GitHub Primer design tokens (light) — shared contract ── */
  --canvas-default: #ffffff;
  --canvas-subtle: #f6f8fa;
  --canvas-inset: #f6f8fa;
  --border-default: #d1d9e0;
  --border-muted: rgba(209, 217, 224, 0.7);
  --fg-default: #1f2328;
  --fg-muted: #59636e;
  --accent-fg: #0969da;
  --success-fg: #1a7f37;
  ...
  /* subtle tinted fills for labels, pills and flashes (Primer muted-bg pairs) */
  ...
  --success-border: rgba(26, 127, 55, 0.35);
  --danger-border: rgba(209, 36, 47, 0.35);
  --attention-border: rgba(154, 103, 0, 0.35);
  --done-border: rgba(130, 80, 223, 0.35);
  --btn-border: rgba(31, 35, 40, 0.15);
  --primary-bg: #1f883d;
  --primary-bg-hover: #1a7f37;
  ...
  --fg: var(--fg-default);
  --fg-subtle: #6e7781;
  --cyan: var(--accent-fg); --cyan-soft: rgba(9, 105, 218, 0.10);
  --green: var(--success-fg); --green-soft: rgba(26, 127, 55, 0.10);
  --amber: var(--attention-fg); --amber-soft: rgba(154, 103, 0, 0.12);
  --red: var(--danger-fg); --red-soft: rgba(209, 36, 47, 0.08);
  --ok: var(--success-fg); --warn: var(--attention-fg); --fail: var(--danger-fg); --run: var(--accent-fg); --queued: var(--fg-subtle);
  --accent: var(--accent-fg);
  --link: var(--accent-fg); --link-hover: #0550ae; --link-shadow: none;
...
/* ── App shell: GitHub settings-style sidebar ── */
...
.brand-icon { object-fit: cover; border: 1px solid var(--border-default); border-radius: 6px; flex: none; }
...
.nav a.active::before {
  ...
  width: 3px; height: 16px; border-radius: 3px; background: var(--accent-fg);
}
...
.btn-primary { background: var(--primary-bg); border-color: var(--btn-border); color: #ffffff; }
...
button.primary { background: var(--primary-bg); color: #fff; border: 1px solid var(--btn-border); font-weight: 500; padding: 0.45rem 0.95rem; text-transform: none; letter-spacing: 0; min-height: 32px; }
...
/* Calm dark theme (toggle opt-in) — GitHub dark canvas, no neon. Components inherit the shared rules. */
:root[data-theme="dark"] {
  ...
  --border-muted: rgba(61, 68, 77, 0.7);
  --fg-default: #f0f6fc;
  --fg-muted: #9198a1;
  --accent-fg: #4493f8;
  --success-fg: #3fb950;
  ...
  --danger-subtle: rgba(248, 81, 73, 0.10);
  --attention-subtle: rgba(187, 128, 9, 0.12);
  --neutral-subtle: rgba(110, 118, 129, 0.15);
  --done: #d2a8ff;
  --done-subtle: rgba(210, 168, 255, 0.12);
  --accent-border: rgba(56, 139, 253, 0.40);
  --success-border: rgba(46, 160, 67, 0.40);
  --danger-border: rgba(248, 81, 73, 0.40);
  --attention-border: rgba(187, 128, 9, 0.40);
  --done-border: rgba(163, 113, 247, 0.40);
  --btn-border: rgba(240, 246, 252, 0.10);
  --primary-bg: #238636;
  --primary-bg-hover: #2ea043;
  --surface-2: #21262d;
  --surface-3: #30363d;
  --border-bright: #6e7681;
  --faint: #6e7681;
  --fg-subtle: #6e7681;
  --bg-over: #30363d;
  --cyan: var(--accent-fg); --cyan-soft: rgba(68, 147, 248, 0.14);
  --green: var(--success-fg); --green-soft: rgba(63, 185, 80, 0.12);
  --amber: var(--attention-fg); --amber-soft: rgba(210, 153, 34, 0.12);
  --red: var(--danger-fg); --red-soft: rgba(248, 81, 73, 0.10);
  --link: var(--accent-fg); --link-hover: #79c0ff;
  --shadow-card: none;
  --shadow-flat: none;
}
