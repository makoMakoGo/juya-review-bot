...
test('metrics trend chart renders an accessible fluid SVG without scroll constraints', () => {
  ...
  assert.match(html, /style="display:block;width:100%;height:auto;max-width:100%"/);
  assert.match(html, /class="metric-chart-jobs-line"/);
  assert.match(html, /class="metric-chart-success-line"/);
  assert.match(html, /class="metric-chart-jobs-area"/);
  assert.match(html, /<title>2026-01-01: 3 jobs<\/title>/);
  assert.match(html, /<title>2026-01-02: 100% success<\/title>/);
  assert.match(html, /Exact values are available in Data view\./);
  assert.doesNotMatch(html, /min-width|overflow-x|width="800"|height="320"/);
});
