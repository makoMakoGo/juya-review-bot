...
test('metrics page renders a shareable scope, balanced responsive trend layout, and exact table', () => {
  ...
  assert.match(html, /grid-template-columns:repeat\(auto-fit,minmax\(132px,1fr\)\)/);
  assert.match(html, /class="metrics-daily-layout"[^>]*grid-template-columns:repeat\(auto-fit,minmax\(min\(100%,420px\),1fr\)\)/);
  assert.match(html, /class="metrics-chart-panel"[^>]*min-width:0/);
  assert.match(html, /class="metrics-daily-table-panel"[^>]*min-width:0/);
  assert.match(html, /class="metrics-chart-scroll"[^>]*overflow-x:auto/);
  ...
  assert.match(html, /aria-current="true" style="background:var\(--accent-subtle\)"/);
});
